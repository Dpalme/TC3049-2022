...
The template method pattern is a behavioral design pattern. We define the algorithm as a two-part component; the first part is the body structure, which doesn't change, and the second part, which is a subset of instructions of the first part but with the ability to be interchangeable, e.i, with one body (first part) you can replace the only instructions that change(second part).
